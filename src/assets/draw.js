...
export function drawGraph(graph,selector="body",params={}) {
    ...
    switch(linkType) {
        case "arrows":
            ...
            svg.append('svg:defs').append('svg:marker')
                ...
                .attr('fill', '#000');               
            paths=svg.selectAll(".link").data(graph.links)
    ...
    paths.style("stroke", link => link.color)
        .attr('class', 'link');

    
    function nodeColor(node) {
        if (node.phantom) return "#F00";
        if (node.istree) return "#00F";
        return "#88F";
    }
    ...
    node.append("title")
        .text(function (d) { return `${d.index}:${d.name} ${strifyNodes(d.dalevel,"index")} tw:${d.twidth}`; });
