...
function xLength(graph) {
    const diffs = graph.links.map(link => Math.abs(link.source.x - link.target.x));
...
export function layout(graph) {
    ...
    if (1) {
        ...
        for (let li = levels.length - 1; li >= 0; li--) {
            ...
            for (let ni = level.length - 1; ni >= 0; ni--) {
                ...
                let nx1 = n.children.length ? Math.min(...n.children.map(m => m.x)) : rightLeaf;
                let nx3 = n.children.length ? Math.max(...n.children.map(m => m.x)) : rightLeaf;
                let nx2 = (nx1 + nx3) / 2;
                let na = n.children.length ? average(n.children.map(m => m.x)) : rightLeaf;
        ...
        levels.forEach(function (level) {
            ...
            level.forEach(function (node) {
                if (node.primaryChildren.length) {
                    ...
                    let leftChild = node.primaryChildren[argmin(node.primaryChildren.map(n => n.x))];
                    let rightChild = node.primaryChildren[argmax(node.primaryChildren.map(n => n.x))];
                    ...
                    if (goalDelta > 0) { // we want to shift left, but how much can we
    ...
    // Report Out
    console.log(`${graph.nodes.length} nodes (w/phantoms), ${graph.links.length} links (w/phantoms)`);
...
export function treeLayout(graph) {
    ...
    levels.forEach(function (level, li) {
        ...
        level.forEach(function (node) {
            node.x = Math.max(left + space(node), node.primaryParent ? node.primaryParent.x : 0);
